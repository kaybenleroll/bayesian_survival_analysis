# Monthly Cashflow Calculation for Life Insurance Policies
# =========================================================
#
# This function calculates monthly premium cashflows from a book of life
# insurance policies, accounting for policy status (inforce/lapsed/completed).
#
# The function handles:
# - Policies that are currently inforce (active)
# - Policies that have lapsed (premiums stop at lapse date)
# - Policies that have completed their term (premiums stop at end date)
...
if (FALSE) {
  ...
  # Example policy data
  policy_tbl <- tibble(
    policy_id = paste0("P", sprintf("%03d", 1:100)),
    policy_status = sample(
      c("inforce", "lapsed", "completed"),
      100,
      replace = TRUE,
      prob = c(0.7, 0.2, 0.1)
      ),
    policy_startdate = seq(
      as.Date("2018-01-01"),
      as.Date("2023-12-31"),
      length.out = 100
      ),
    policy_enddate = policy_startdate + years(10),
    prem_ape = runif(100, min = 600, max = 3600)
    ) |>
    mutate(
      policy_statuschangedate = if_else(
        policy_status == "lapsed",
        policy_startdate + days(sample(365:2555, n(), replace = TRUE)),
        as.Date(NA)
        ),
      policy_statuschangedate = if_else(
  ...
  # Generate cashflow dates (e.g., first day of each month)
  cashflow_dates <- seq(
    as.Date("2024-01-01"),
    as.Date("2026-12-31"),
    by = "month"
  ...
  # Calculate cashflows
  cashflow_tbl <- calculate_monthly_cashflows(
    policy_tbl,
    cashflow_dates
    )

  # Summarize
  cashflow_summary_tbl <- summarize_monthly_cashflows(cashflow_tbl)

  # Add cumulative
  cashflow_cumulative_tbl <- calculate_cumulative_cashflows(cashflow_summary_tbl)

  # View results
  cashflow_summary_tbl |> glimpse()

  cashflow_cumulative_tbl |>
    tail(10) |>
    print()

  # Plot
  plot_monthly_cashflows(cashflow_summary_tbl)

  # Policy-specific analysis
  policy_cashflow_tbl <- cashflow_tbl |>
    group_by(policy_id, policy_status) |>
    summarise(
      total_cashflow = sum(cashflow),
      periods_active = sum(is_active),
      first_payment  = min(cashflow_date[is_active]),
      last_payment   = max(cashflow_date[is_active]),

      .groups = "drop"
      )

  policy_cashflow_tbl |> glimpse()
}
